(function game() {
  ...
  function processFireBalls() {
    scene.fireBalls.forEach((fbe) => {
      const newX =
        config.speed * config.fireballMovingMultiplier +
        utils.pxToNumber(fbe.style.left);
  ...
  function processNextRenderQueue() {
    gameplay.nextRenderQueue = gameplay.nextRenderQueue.reduce(
      (acc, currFn) => {
        if (currFn()) {
          return acc;
        }
  ...
  function bugElementProcessor(bugEl) {
    const fireball = scene.fireBalls.find((fe) =>
      utils.hasCollision(fe, bugEl)
    );
    if (fireball) {
      fireball.remove();
      bugEl.remove();
      gameScoreValueEl.innerText =
        config.bugKillScore + +gameScoreValueEl.innerText;
      return true;
    }
